...
from pwnagotchi.ui.controller import UIController
...
class Agent(Client, Automata, AsyncAdvertiser):
    def __init__(self, view, config, keypair):
        ...
        self.mode = 'manual'  # Start in manual mode for UI control

        # Initialize UI controller for interactive mode
        self._ui_controller = None
        if config.get('ui', {}).get('interactive', False):
            self._ui_controller = UIController(self, view, config)
            if hasattr(view, 'set_ui_controller'):
                view.set_ui_controller(self._ui_controller)
    ...
    def start_ui_controller(self):
        """Start the interactive UI controller"""
        if self._ui_controller:
            self._ui_controller.start()
            logging.info("Interactive UI controller started")

    def stop_ui_controller(self):
        """Stop the interactive UI controller"""
        if self._ui_controller:
            self._ui_controller.stop()
            logging.info("Interactive UI controller stopped")
