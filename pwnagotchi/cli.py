...
from pwnagotchi.ui.interactive_view import InteractiveView
...
def pwnagotchi_cli():
    ...
    parser.add_argument('--interactive', dest="do_interactive", action="store_true", default=False, help="Interactive touchscreen mode.")
    ...
    def do_interactive_mode(agent):
        logging.info("entering interactive mode ...")

        agent.mode = 'manual'
        agent.last_session.parse(agent.view(), args.skip_session)

        # Start the UI controller for touch interface
        agent.start_ui_controller()
        ...
        try:
            while True:
                ...
                # Check if UI controller is still running
                ui_controller = agent.get_ui_controller()
                if ui_controller:
                    status = ui_controller.get_status()

                    # Update display based on current mode
                    if status['mode'] == 'pawning' and status['pawning_active']:
                        # Pawning is active, let it continue
                        pass
                    elif status['mode'] == 'bluetooth' and status['bluetooth_scanning']:
                        # Bluetooth scanning is active
                        pass
                    else:
                        # In menu or idle mode
                        display.on_manual_mode(agent.last_session)
            ...
            logging.info("Interactive mode interrupted by user")
        finally:
            agent.stop_ui_controller()
    ...
    # Use interactive view if in interactive mode or if configured
    if args.do_interactive or config.get('ui', {}).get('interactive', False):
        config['ui']['interactive'] = True
        display = InteractiveView(config=config,
                                impl=display._implementation,
                                state={'name': '%s>' % pwnagotchi.name()})
    ...
    def usr1_handler(*unused):
        ...
        agent._restart("MANU" if args.do_manual or args.do_interactive else "AUTO")

    signal.signal(signal.SIGUSR1, usr1_handler)

    if args.do_interactive:
        do_interactive_mode(agent)
    elif args.do_manual:
